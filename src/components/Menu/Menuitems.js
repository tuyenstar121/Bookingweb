...
const MenuItems = ({ items, onAddToCart,promotionToday } ) => {
  ...
  const getDiscountPercentage = (itemId, promotions) => {
    if (!promotions || promotions.length === 0) {
      return 0; // Không có khuyến mãi nào
    }
    const promotion = promotions.find((p) => p.itemId === itemId);
    return promotion ? promotion.discount : 0; // Trả về giảm giá nếu có
  };
  ...
  return (
    <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
      {items.map((item) => {
        ...
        const discountPercentage = getDiscountPercentage(id);
        return (
          <article key={id} className="relative flex flex-col bg-white rounded-lg overflow-hidden shadow-lg border">
